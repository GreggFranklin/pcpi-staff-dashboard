...
private static function resolve_workflow_key_from_entry( array $entry, array $cfg = [] ): string {
	...
	// 0) Hidden field with admin label "workflow_key" on the entry's form.
	//    Only applies to questionnaire-root / kiosk workflows (e.g. WADA) where the
	//    questionnaire form itself carries the workflow_key hidden field because there
	//    is no preceding applicant form.
	//    Skipped when the entry belongs to an applicant form — those workflows resolve
	//    the key via the field 1005 dropdown in Step 1 below.
	static $workflow_key_field_cache = [];
	$form_id = absint( rgar( $entry, 'form_id' ) );

	$is_applicant_form = false;
	if ( class_exists( 'PCPI_Workflow_Engine' ) && method_exists( 'PCPI_Workflow_Engine', 'get_workflows' ) ) {
		foreach ( (array) PCPI_Workflow_Engine::get_workflows() as $wf ) {
			$wf = (array) $wf;
			if ( ! empty( $wf['applicant_form_id'] ) && absint( $wf['applicant_form_id'] ) === $form_id ) {
				$is_applicant_form = true;
				break;
			}
		}
	}

	if ( ! $is_applicant_form && $form_id > 0 && class_exists( 'GFAPI' ) ) {
...
$applicant_display_html = esc_html( $applicant_display );
			...
			// -----------------------------------------------------------------
			// has_review: registry-driven flag that controls whether an examiner
			// review step is required before Summary/Send PDF become available.
			//
			// Derivation order (first match wins):
			//   1. Explicit 'has_review' key in the registry entry.
			//   2. Implicit: review_form_id is present  -> has_review = true.
			//   3. Implicit: kiosk workflow             -> has_review = false.
			//   4. Default: true (preserves legacy behaviour for review workflows).
			//
			// When has_review = false, Summary and Send PDF unlock as soon as the
			// questionnaire is submitted (PDF is built from the questionnaire entry,
			// not a review entry). Review button is disabled with an appropriate title.
			// -----------------------------------------------------------------
			if ( array_key_exists( 'has_review', $workflow ) ) {
				$has_review = (bool) $workflow['has_review'];
			} elseif ( ! empty( $workflow['review_form_id'] ) ) {
				$has_review = true;
			} elseif ( $is_kiosk ) {
				$has_review = false;
			} else {
...
if ( $is_kiosk ) {
	...
	// No-review workflows: the Review step is not part of this workflow.
	$review_btn = '<span class="gf-action-btn pcpi-btn-review pcpi-btn-disabled gf-disabled disabled" aria-disabled="true" title="Review is not used for this workflow">Review</span>';
 ...
 */
			...
			} elseif ( ! $has_review && ! $is_kiosk ) {
				// No-review, non-kiosk workflow: build PDF directly from the questionnaire entry.
				// Mirrors the kiosk path above; no review entry is needed.
				if ( $q_entry_id > 0 && $row_pdf_id ) {
					if ( function_exists( 'pcpi_build_secure_pdf_link' ) ) {
						$pdf_url = (string) pcpi_build_secure_pdf_link( $q_entry_id, (string) $row_pdf_id );
					} else {
	...
	" review_entry_id=$review_entry_id"
			...
			if ( $pdf_ready && $pdf_url !== '' ) {
				...
				if ( $is_kiosk || ! $has_review ) {
					$summary_btn = '<span class="gf-action-btn gf-disabled pcpi-btn-disabled" title="Summary will be available after the form is submitted">Summary</span>';
				} else {
			...
			} elseif ( $pdf_ready && ( $review_entry_id > 0 || ! $has_review ) ) {
				// Active for both review workflows (review_entry_id > 0) and
				// no-review workflows (pdf built from questionnaire entry; review_entry_id = 0).
